...
Check the statistically significant cycle
...
```{r}
reconstruct_ts(countries_wavelets, period = seq(1.5, 2.5, 0.1))
```
...
plot_coherence_power <- function(coherence_list){
  # return heatmap of power and also global power
...
```{r}
plot_coherence_power(countries_coherence)
```
...
```{r}
compute_phase_diff(countries_wavelets, period = 4.5)$plot / 
compute_phase_diff_alt(countries_coherence, period = 4.5)$plot
```
